...
=<
  ...
  ++  on-agent
    ...
    ::
        %fact
      ?.  ?=(%group-update p.cage.gift)
        (on-agent:def wire gift)
      =^  cards  state
        ?:  (team:title our.bowl src.bowl)
          (handle-local:gc !<(group-update q.cage.gift))
        (handle-foreign:gc !<(group-update q.cage.gift))
      [cards this]
    ==
  --
::
